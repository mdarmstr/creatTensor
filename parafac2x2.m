...
switch size(varargin,2)
    ...
    case 2
        ...
        [~,init_indx] = min(ssr(end,:));
        [F,A,D,pvar,ssr] = nnparafac2x2als(Xijkl,R,1e-6,1000,1,0,Bkli(:,:,:,init_indx),Dkli(:,:,:,init_indx),Akli(:,:,init_indx),Bskli(:,:,init_indx),Bili(:,:,:,init_indx),Dili(:,:,:,init_indx),Aili(:,:,init_indx),Bsili(:,:,init_indx));
...
while abs(ssr1 - ssr2) > eps && abs(ssr1 - ssr2)/abs(ssr2) > eps && iter < maxiter
    ...
    for kl = 1:KL
        Dkl(:,:,kl) = diag(diag(pinv(Akl'*Akl)*(Akl'*Xkl(:,:,kl)'*Bkl(:,:,kl))*psqrt(Bkl(:,:,kl)'*Bkl(:,:,kl))));
    end
    ...
    %Dil Estimation
    for il = 1:sz(1)*sz(4)
        Dil(:,:,il) = diag(diag(pinv(Ail'*Ail)*(Ail'*Xil(:,:,il)'*Bil(:,:,il))*psqrt(Bil(:,:,il)'*Bil(:,:,il))));
    end
